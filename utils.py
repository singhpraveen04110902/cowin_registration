    ...
    try:
        ...
        for location in location_dtls:
            ...
            if resp.status_code == 401:
                ...
                resp = resp.json()
                if 'centers' in resp:
                    print(f"Centers available in {location['district_name']} from {start_date} as of {today.strftime('%Y-%m-%d %H:%M:%S')} dose={dose}: {len(resp['centers'])}")
    ...
    try:
        ...
        for location in location_dtls:
            ...
            if resp.status_code == 401:
                ...
                resp = resp.json()
                if 'centers' in resp:
                    print(f"Centers available in {location['pincode']} from {start_date} as of {today.strftime('%Y-%m-%d %H:%M:%S')} dose={dose}: {len(resp['centers'])}")
                    options += viable_options(resp, minimum_slots, min_age_booking, fee_type, dose)
